...
/// 均可还原。
///
/// DISCOVERABILITY NOTE: this page lives at Settings → 回收站, and every
/// delete flow now mentions that path in its confirmation dialog, so items
/// no longer vanish without a trace.
///
/// 可发现性说明：本页位于「设置 → 回收站」，且所有删除流程的确认对话框
/// 都会提到该路径，条目不会再无迹可寻地消失。
...
class _StickerRow extends ConsumerWidget {
  ...
  Future<void> _restore(WidgetRef ref, BuildContext context) async {
    ...
    // `contextIsAlive` is this project's mounted-guard for pre-3.4 Flutter,
    // where BuildContext.mounted does not exist yet.
    // `contextIsAlive` 是本项目在 Flutter 3.4 之前（尚无 BuildContext.mounted）
    // 使用的 mounted 守卫。
...
class _SeriesRow extends ConsumerWidget {
  const _SeriesRow({required this.series, required this.busy});
